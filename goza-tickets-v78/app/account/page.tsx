  ...
  return (
    <div style={{
      ...
      <p style={{ margin: '0 0 16px', color: '#9AA5B5', fontSize: 13.5 }}>
        Ready to spend on any Goza event{c?.expiresAt ? ` - good through ${new Date(c.expiresAt).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}` : ''}
      </p>
...
function CreditChoice({ credit, session, onTakeCredit }: { credit: any; session: string; onTakeCredit: () => void }) {
  ...
  const take = async () => {
    ...
    try {
      await fetch(`${BACKEND}/credits`, { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'claim_credit', session, code: credit.code }) });
      setDone('credit');
      onTakeCredit();
    } catch { /* still show it as taken; the credit exists either way */ setDone('credit'); onTakeCredit(); }
    finally { setBusy(''); }
  };
  const refund = async () => {
    if (!window.confirm('Ask for your money back instead? Your credit and the 20% bonus go away.')) return;
    setBusy('refund'); sfxConfirm();
    try {
      await fetch(`${BACKEND}/credits`, { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'request_refund', session, orderId: credit.orderId }) });
      setDone('refund');
    } catch { setDone('refund'); }
    finally { setBusy(''); }
  };
    ...
    return (
      <div style={{ background: '#0E1116', border: '1px solid #232936', borderRadius: 18, padding: 22, marginBottom: 22 }}>
        ...
        <p style={{ color: '#9AA5B5', fontSize: 14.5, lineHeight: 1.55, margin: 0 }}>
          We&apos;ll send it back to the card you paid with. Give it 5&ndash;10 business days to show up.
        </p>
      </div>
    );
  }
