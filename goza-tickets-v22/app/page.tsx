...
export default function Home() {
  ...
  useEffect(() => {
    trackPageView();
    api('/checkout?browse=1').then((d) => { setEvents(d.events); if (d.accentColor) setSiteAccent(d.accentColor); }).catch((e) => setErr(e.message));
  }, []);
    ...
    <div style={{ minHeight: '100vh', background: 'radial-gradient(120% 90% at 50% 0%, #2a1018 0%, #0c0508 55%, #000 100%)', backgroundAttachment: 'fixed', fontFamily: F }}>
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: '28px 18px 60px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
          <div>
            <p style={{ color: siteAccent, fontSize: 12, letterSpacing: 3, fontWeight: 700, margin: '0 0 4px' }}>GOZA ENTERTAINMENT</p>
            <h1 style={{ color: '#fff', fontSize: 32, fontWeight: 800, margin: 0, letterSpacing: -0.5 }}>Upcoming events</h1>
          </div>
          <button onClick={askLocation}
            style={{ background: sortBy === 'near' ? siteAccent : 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.16)', color: '#fff', borderRadius: 22, padding: '10px 18px', fontSize: 14, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7 }}>
          ...
          {list.map((e) => (
            <a key={e.id} href={`/e?id=${e.id}`} className="gz-card"
              ...
              <div style={{ padding: '14px 16px 16px' }}>
                <p style={{ color: e.accentColor || siteAccent, fontSize: 12.5, fontWeight: 700, margin: '0 0 6px', letterSpacing: 0.3 }}>{fmtDate(e.date)}</p>
