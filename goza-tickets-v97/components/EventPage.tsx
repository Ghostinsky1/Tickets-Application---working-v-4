...
import { sfxClick, sfxCoin } from '@/lib/sfx';
import { api, money, fmtDate, ORGANIZER, DEFAULT_EVENT_ID, BACKEND } from '@/lib/api';
import CardCheckout from './CardCheckout';
import { trackViewContent, trackInitiateCheckout, trackAddToCart, trackAddPaymentInfo, identifyUser, trackPageView, trackLead } from '@/lib/track';
...
type Step = null | 'phone' | 'code' | 'contact' | 'cart' | 'pay';
...
export default function EventPage({ eventId = DEFAULT_EVENT_ID }: { eventId?: string }) {
  ...
  const [seatTierIds, setSeatTierIds] = useState<string[]>([]);
  useSeatTiers(eventId, setSeatTierIds);
  ...
  const sendCode = async () => {
    ...
    try {
      const r = await fetch(`${BACKEND}/leads`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'send_code', eventId, phone: `+1${phoneDigits(phone)}` }) }).then((x) => x.json());
      if (r.verified || r.skip) { afterVerify(); return; }
      if (r.error) { setOtpErr(r.error); if (step !== 'code') setStep('code'); return; }
      setResendIn(30);
      if (step !== 'code') setStep('code');
    } catch { afterVerify(); } // network fail: never block
    finally { setOtpBusy(false); }
  };
  const checkCode = async (code: string) => {
    setOtpBusy(true); setOtpErr('');
    try {
      const r = await fetch(`${BACKEND}/leads`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'check_code', phone: `+1${phoneDigits(phone)}`, code }) }).then((x) => x.json());
      if (r.verified) { afterVerify(); return; }
      setOtpErr(r.error || 'Wrong code');
    } catch (e: any) { setOtpErr(e.message); }
    finally { setOtpBusy(false); }
  };
  const [selSeats, setSelSeats] = useState<string[]>([]);
  const [selSeatLabels, setSelSeatLabels] = useState<string[]>([]);
  const [seatFace, setSeatFace] = useState(0);
  const multi = false; // single unified flow
  const [multiQuote, setMultiQuote] = useState<any>(null);
  const [ttId, setTtId] = useState('');
  const [hiddenTiers, setHiddenTiers] = useState<string[]>([]);
  useEffect(() => { fetch(`${BACKEND}/plink?hidden=${eventId}`).then((r) => r.json()).then((d) => { if (Array.isArray(d.hidden)) setHiddenTiers(d.hidden); }).catch(() => {}); }, [eventId]);
  // MEMBER SESSION: if they've signed in before on this device, we already know
  // who they are - prefill everything and skip straight to the cart.
  const [member, setMember] = useState<any>(null);
  useEffect(() => {
    let sess = '';
    try { sess = localStorage.getItem('gz_portal_session') || ''; } catch {}
    if (!sess) return;
    fetch(`${BACKEND}/account?session=${encodeURIComponent(sess)}`)
      .then((r) => r.json())
      .then((d) => {
        if (d?.ok && d.customer?.phone) {
          setMember(d.customer);
          setName(d.customer.name || '');
          setEmail(d.customer.email || '');
          setPhone(String(d.customer.phone).replace(/^\+1/, ''));
          try { localStorage.setItem('gz_vphone', String(d.customer.phone).replace(/\D/g, '').slice(-10)); } catch {}
        }
      })
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    try {
      ...
      if (p) { localStorage.setItem(`gz_promo_${eventId}`, p); setPromoterId(p); }
      else setPromoterId(localStorage.getItem(`gz_promo_${eventId}`) || '');
    } catch {}
  }, [eventId]);
  const [captured, setCaptured] = useState(() => { try { return !!localStorage.getItem(`gz_cap_${eventId}`) || !!sessionStorage.getItem(`gz_capskip_${eventId}`); } catch { return false; } });
  const seated = seatTierIds.includes(ttId);
  const [qty, setQty] = useState(1);
  const [addonOn, setAddonOn] = useState(false);
  const [quote, setQuote] = useState<Quote | null>(null);
  const [loadingQuote, setLoadingQuote] = useState(false);
  const [phone, setPhone] = useState('');
  // After the number is verified: if we already know them, fill everything in and
  // jump to the cart. Only a genuinely new buyer gets asked for name + email.
  const afterVerify = useCallback(async () => {
    try { localStorage.setItem('gz_vphone', phoneDigits(phone)); } catch {}
    try {
      const d = await fetch(`${BACKEND}/account?phone=${encodeURIComponent(`+1${phoneDigits(phone)}`)}`).then((r) => r.json());
      if (d?.found && d.name && d.email) {
        setName(d.name);
        setEmail(d.email);
        setStep('cart');
        return;
    ...
    } catch { /* lookup failed: just ask them */ }
    setStep('contact');
  }, [phone]);

  const [name, setName] = useState('');
  const [email, setEmail] = useState('');
  // store credit the signed-in member can spend here
  ...
  // anyone can paste a credit code - no sign-in needed, it's validated against
  // the phone they're checking out with
  const applyTypedCode = async () => {
    const code = typedCode.trim();
    if (!code) return;
    setCodeBusy(true); setCodeMsg('');
    try {
      const d = await fetch(`${BACKEND}/cart`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
        action: 'quote', eventId, items: [{ ticketTypeId: ttId, quantity: qty }],
        creditCode: code, phone: `+1${phoneDigits(phone)}`,
      }) }).then((r) => r.json());
      if (d?.credit?.error) { setCodeMsg(d.credit.error); return; }
      if (!d?.credit) { setCodeMsg('That code did not work'); return; }
      setMyCredit({ code, balance: money(d.credit.available) });
      setUseCredit(true);
      setCreditQuote(d);
      setCodeOpen(false);
      setCodeMsg('');
    } catch { setCodeMsg('Could not check that code right now'); }
    finally { setCodeBusy(false); }
  };
  const [creditQuote, setCreditQuote] = useState<any>(null);
  useEffect(() => {
    let sess = '';
    try { sess = localStorage.getItem('gz_portal_session') || ''; } catch {}
    if (!sess) return;
    fetch(`${BACKEND}/credits`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'my_credits', session: sess }) })
      .then((r) => r.json())
      .then((d) => { if (d?.ok && d.totalCents > 0) setMyCredit(d.credits[0]); })
      .catch(() => {});
  }, []);
  const lastLeadSent = useRef('');
  useEffect(() => {
    if (step !== 'contact') return;
    const nm = name.trim(); const em = email.trim().toLowerCase();
    if (nm.length < 2 || !/^[^@\s]+@[^@\s]+\.[^@\s]{2,}$/.test(em)) return;
    const key = `${nm}|${em}`;
    if (key === lastLeadSent.current) return;
    const t = setTimeout(() => {
      lastLeadSent.current = key;
      try { fetch(`${BACKEND}/leads`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ eventId, phone: `+1${phoneDigits(phone)}`, name: nm, email: em }), keepalive: true }); } catch {}
    }, 800);
    return () => clearTimeout(t);
  }, [step, name, email, phone, eventId]);
  ...
  useEffect(() => { if (step === 'cart' && !seated) fetchQuote(ttId, qty, discount, addonOn); }, [step, ttId, qty, discount, addonOn, fetchQuote, seated]);
  useEffect(() => {
    if (step !== 'cart' || seated || !myCredit || !useCredit || !ttId) { setCreditQuote(null); return; }
    let dead = false;
    fetch(`${BACKEND}/cart`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
      action: 'quote', eventId, items: [{ ticketTypeId: ttId, quantity: qty }],
      creditCode: myCredit.code, phone: `+1${phoneDigits(phone)}`,
    }) }).then((r) => r.json()).then((d) => { if (!dead && d?.credit && !d.credit.error) setCreditQuote(d); }).catch(() => {});
    return () => { dead = true; };
  }, [step, seated, myCredit, useCredit, ttId, qty, eventId, phone]);
  useEffect(() => {
    if (step !== 'cart' || !seated || !selSeats.length) { if (seated) setMultiQuote(null); return; }
    let dead = false;
    fetch(`${BACKEND}/cart`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'quote', eventId, items: [{ ticketTypeId: ttId, quantity: selSeats.length }] }) })
      .then((r) => r.json()).then((d) => { if (!dead && !d.error) setMultiQuote(d); }).catch(() => {});
    return () => { dead = true; };
  }, [step, seated, selSeats.length, ttId, eventId]);
  ...
  const types = (info?.ticketTypes ?? []).filter((t: any) => !hiddenTiers.includes(t.id));
  useEffect(() => {
    if (!types.length) return;
    const current = types.find((t: any) => t.id === ttId);
    if (!current || current.remaining <= 0) {
      const next = types.find((t: any) => t.remaining > 0);
      if (next && next.id !== ttId) setTtId(next.id);
    }
  }, [JSON.stringify(types.map((t: any) => [t.id, t.remaining])), ttId]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const pay = async () => {
    ...
    try {
      // SEATED (VIP) orders: hold the chosen seats, then one hosted checkout — same
      // contact info, same flow, seats locked to this buyer for the payment window.
      if (seated) {
        if (!selSeats.length) { setError('Pick your seats first'); setSubmitting(false); return; }
        let hk = 'anon'; try { hk = localStorage.getItem('gz_seat_key') || (localStorage.setItem('gz_seat_key', Math.random().toString(36).slice(2)), localStorage.getItem('gz_seat_key')!); } catch {}
        const h = await api('/seats', { method: 'POST', body: JSON.stringify({ action: 'hold', eventId, seatIds: selSeats, holdKey: hk }) });
        if (!h.ok) { setError(h.error || 'Those seats just got taken — pick again'); setSelSeats([]); setSelSeatLabels([]); setSubmitting(false); return; }
        const d = await api('/seats', { method: 'POST', body: JSON.stringify({ action: 'checkout', eventId, seatIds: selSeats, holdKey: hk, buyerName: name.trim(), buyerEmail: email.trim(), buyerPhone: `+1${phoneDigits(phone)}`, ...(promoterId ? { promoterId } : {}) }) });
        if (d.url) { window.location.href = d.url; return; }
        setError(d.error || 'Could not start checkout'); setSubmitting(false); return;
      }
      // credit applied -> cart path (it reserves the credit and charges the remainder)
      if (myCredit && useCredit && creditQuote?.credit && creditQuote.credit.applied > 0) {
        const d = await api('/cart', { method: 'POST', body: JSON.stringify({
          action: 'checkout', eventId,
          items: [{ ticketTypeId: ttId, quantity: qty }],
          buyerName: name.trim(), buyerEmail: email.trim(), buyerPhone: `+1${phoneDigits(phone)}`,
          creditCode: myCredit.code,
          ...(promoterId ? { promoterId } : {}),
        }) });
        if (d.url) { window.location.href = d.url; return; }
        // credit covered the whole thing: the order already exists, celebrate and go
        if (d.paidWithCredit && d.redirect) { sfxCoin(); window.location.href = d.redirect; return; }
        setError(d.error || 'Could not start checkout'); setSubmitting(false); return;
      }
      const payload = {
  ...
  // CAPTURE MODE — signup gate first (incentive headline set in admin), then the
  // buyer lands STRAIGHT in the cart with their info prefilled. Lead captured
  // either way -> follow-up list + Blast Center.
  if (ev.eventMode === 'capture' && !captured) {
    return <CaptureGate ev={ev} btn={btn} accentCol={accentCol} accent={accent} deep={deep} onDone={(ph) => {
      setPhone(ph);
      try { localStorage.setItem(`gz_cap_${ev.id}`, '1'); localStorage.setItem('gz_vphone', ph.replace(/\D/g, '').slice(-10)); } catch {}
      identifyUser('', `+1${ph.replace(/\D/g, '').slice(-10)}`);
      setCaptured(true);
      setStep('cart');
    }} onSkip={() => {
      try { sessionStorage.setItem(`gz_capskip_${ev.id}`, '1'); } catch {}
      setCaptured(true); // this visit only — no localStorage, they get asked again next time
    }} />;
  }
  ...
  const open = () => {
    ...
    sfxClick();
    setError('');
    trackInitiateCheckout(eventId, ev.name);
    // signed-in member with everything on file -> straight to the cart
    if (member && member.phone && member.email && member.name) { setStep('cart'); return; }
    setStep('phone');
  };

  return (
    <div style={{ minHeight: '100vh', fontFamily: F, background: '#000' }}>
      <style>{`
        ...
        <div style={{ paddingBottom: 40 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '16px 0 14px' }}>
            {ev.logoUrl
              ...
              : <>
                  <GozaStar color={accentCol} size={28} />
                  <span style={{ color: '#fff', fontFamily: FL, fontSize: 12, fontWeight: 700, letterSpacing: 2.2, textTransform: 'uppercase' }}>{ORGANIZER.name}</span>
                </>}
          ...
          {details.length > 0 && (
            <>
              ...
              <div style={{ background: CARD, backdropFilter: 'blur(8px)', borderRadius: 16, padding: 20, marginBottom: 28, border: '1px solid rgba(255,255,255,0.07)' }}>
                {/* Collapsed by default: first 2 lines, faded at the cut, with a More info
                    toggle. Short descriptions (<=2 lines) never show the toggle. */}
                {/* mask-based fade: works on any event colour, no painted black bar */}
                <div style={{
                  maxHeight: detailsOpen ? 'none' : 112,
                  overflow: 'hidden',
                  ...(!detailsOpen && details.length > 2 ? {
                    WebkitMaskImage: 'linear-gradient(180deg, #000 58%, rgba(0,0,0,0.15) 100%)',
                    maskImage: 'linear-gradient(180deg, #000 58%, rgba(0,0,0,0.15) 100%)',
                  } : {}),
                ...
                {details.length > 2 && (
                  <button onClick={() => setDetailsOpen(!detailsOpen)}
                    style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 999, color: accentCol, fontFamily: FL, fontSize: 12.5, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', cursor: 'pointer', padding: '10px 16px', margin: '14px 0 0', display: 'inline-flex', alignItems: 'center', gap: 7 }}>
                    {detailsOpen ? 'Less info' : 'More info'}
                    <span style={{ fontSize: 11, transform: detailsOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.15s' }}>▼</span>
                  </button>
                )}
              </div>
            </>
          ...
          {ev.hideVenue ? (
            ...
            <EventMap location={ev.location} accent={accent} accentCol={accentCol} />
          )}
          ...
          <div style={{ width: '100%', maxWidth: 520, background: '#0b0b0b', borderRadius: '22px 22px 0 0', maxHeight: '92vh', display: 'flex', flexDirection: 'column', overscrollBehavior: 'contain' }} onClick={(e) => e.stopPropagation()}>
            ...
            {step === 'phone' && (
              <>
                ...
                <p style={{ color: '#9AA5B5', fontSize: 14, lineHeight: 1.5, margin: '0 0 16px' }}>
                  We text your confirmation + receipt here after you pay.
                </p>
                <input
                  value={phone}
                  ...
                  onClick={() => {
                    try {
                      fetch(`${BACKEND}/leads`, {
                        ...
                        body: JSON.stringify({ eventId, phone: `+1${phoneDigits(phone)}` }),
                        keepalive: true,
                      });
                    } catch {}
                    let v = '';
                    try { v = localStorage.getItem('gz_vphone') || ''; } catch {}
                    if (v === phoneDigits(phone)) { afterVerify(); return; }
                    setOtpCode('');
                    sendCode();
                  }}
                  disabled={!phoneOk || otpBusy}
                  style={{ ...PRIMARY, background: btn, color: '#fff', opacity: phoneOk && !otpBusy ? 1 : 0.45 }}
                >
            ...
            {step === 'code' && (
              <div>
                <h3 style={{ color: '#fff', fontSize: 20, fontWeight: 800, margin: '0 0 6px' }}>Enter your code</h3>
                <p style={{ color: '#8a8f98', fontSize: 13.5, margin: '0 0 16px' }}>We texted a 6-digit code to ({phoneDigits(phone).slice(0, 3)}) {phoneDigits(phone).slice(3, 6)}-{phoneDigits(phone).slice(6)}</p>
                <input value={otpCode} autoFocus inputMode="numeric" maxLength={6} placeholder="123456"
                  onChange={(e) => { const v = e.target.value.replace(/\D/g, '').slice(0, 6); setOtpCode(v); setOtpErr(''); if (v.length === 6) checkCode(v); }}
                  style={{ width: '100%', boxSizing: 'border-box', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.14)', borderRadius: 12, color: '#fff', fontSize: 28, fontWeight: 800, letterSpacing: 10, textAlign: 'center', padding: '14px 0', outline: 'none', marginBottom: 10 }} />
                {otpErr && <p style={{ color: '#ff6b6b', fontSize: 13, margin: '0 0 10px' }}>{otpErr}</p>}
                {otpBusy && <p style={{ color: '#8a8f98', fontSize: 13, margin: '0 0 10px' }}>Checking…</p>}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <button onClick={() => { setStep('phone'); setOtpCode(''); setOtpErr(''); }} style={BACKBTN}>Back</button>
                  <button onClick={() => { if (resendIn <= 0) { setOtpCode(''); sendCode(); } }} disabled={resendIn > 0}
                    style={{ background: 'none', border: 'none', color: resendIn > 0 ? '#6f747d' : accentCol, fontSize: 13.5, fontWeight: 700, cursor: resendIn > 0 ? 'default' : 'pointer' }}>
            ...
            {step === 'contact' && (
              <>
                ...
                <p style={{ color: '#9AA5B5', fontSize: 14, lineHeight: 1.5, margin: '0 0 14px' }}>
                  Verified {phone} &mdash; last bit and you&apos;re done.
                </p>
            ...
            {step === 'cart' && (
              <>
                ...
                {member && (
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, background: 'rgba(17,64,240,0.10)', border: '1px solid rgba(17,64,240,0.35)', borderRadius: 12, padding: '10px 13px', marginBottom: 14 }}>
                    <span style={{ color: '#fff', fontSize: 13.5 }}>
                      Signed in as <strong>{(member.name || '').split(' ')[0] || 'you'}</strong>
                      <span style={{ color: '#8a8f98' }}> · {member.email}</span>
                    </span>
                    <button onClick={() => { setMember(null); setStep('phone'); }}
                      style={{ background: 'none', border: 'none', color: accentCol, fontSize: 12.5, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' }}>
                      Not you?
                    </button>
                  </div>
                )}
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
                    {types.map((t) => (
                      <button key={t.id} onClick={() => setTtId(t.id)} disabled={t.remaining <= 0}
                        ...
                        {t.name} · {money(t.price)}{seatTierIds.includes(t.id) ? ' · PICK YOUR SEAT' : ''}{t.remaining <= 0 ? ' · SOLD OUT' : ''}
                      </button>
                    ))}
                  </div>
                )}
                ...
                {seated ? (
                  <SeatPicker eventId={eventId} accent={accentCol} tierName={types.find((t) => t.id === ttId)?.name || 'VIP'}
                    selected={selSeats} onChange={(ids, labels, total) => { setSelSeats(ids); setSelSeatLabels(labels); setSeatFace(total); }} />
                ) : (
                ...
                {!seated && bump && !bumpActive && qty >= bump.triggerQty && (
                ...
                <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: 12, marginBottom: 8 }}>
                  {myCredit && !seated && (
                    <div style={{ background: 'rgba(17,64,240,0.10)', border: '1px solid rgba(17,64,240,0.4)', borderRadius: 12, padding: '12px 14px', marginBottom: 12 }}>
                      <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
                        <input type="checkbox" checked={useCredit} onChange={(e) => setUseCredit(e.target.checked)} style={{ width: 18, height: 18, accentColor: '#1140F0' }} />
                        <span style={{ color: '#fff', fontSize: 14, fontWeight: 600 }}>
                          Use my Goza credit <span style={{ color: '#8FB4FF' }}>({myCredit.balance} available)</span>
                        </span>
                      </label>
                      {useCredit && creditQuote?.credit && (
                        <p style={{ color: '#8FB4FF', fontSize: 12.5, margin: '8px 0 0' }}>
                          −{money(creditQuote.credit.applied)} applied
                          {creditQuote.credit.remainingAfter > 0 ? ` · ${money(creditQuote.credit.remainingAfter)} left over` : ' · uses it all'}
                        </p>
                      )}
                    </div>
                  )}
                  ...
                  {seated ? (
                    selSeats.length === 0 ? <p style={{ color: '#8a8f98', fontSize: 13, margin: 0 }}>Tap your seats above 👆</p>
                    : multiQuote ? (
                      <div>
                        <Row l={`Seat${selSeats.length > 1 ? 's' : ''} ${selSeatLabels.join(', ')}`} v={money(seatFace)} />
                        <div style={{ height: 1, background: 'rgba(255,255,255,0.08)', margin: '8px 0' }} />
                        <Row l="Total incl. fees" v={money(multiQuote.total)} big />
                      </div>
                    ) : <p style={{ color: '#8a8f98', fontSize: 13 }}>Calculating…</p>
                  ) : quote ? (
                    <div style={{ opacity: loadingQuote ? 0.55 : 1, transition: 'opacity 0.12s' }}>
                      ...
                      {useCredit && creditQuote?.credit ? (
                        <>
                          <Row l="Goza credit" v={`−${money(creditQuote.credit.applied)}`} />
                          <Row l="Due now" v={money(creditQuote.credit.dueNow)} big />
                        </>
                      ) : (
                ...
                <button onClick={() => setStep('phone')} disabled={submitting} style={BACKBTN}>Back</button>
...
/* ---------- CAPTURE GATE (phone-only, styled 1:1 like the drop teaser) ---------- */
/* GOZA OS star mark - takes whatever colour it's given, so on a ticket page it
   wears that event's flyer colour instead of the brand blue */
function GozaStar({ color = '#1140F0', size = 28 }: { color?: string; size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" aria-hidden="true" style={{ display: 'block', flexShrink: 0, filter: `drop-shadow(0 0 10px ${color}66)` }}>
      <path fill={color} d="M56 4 L64 34 L97 31 L68 50 L80 90 L50 68 L20 92 L32 55 L3 44 L38 38 Z" />
    </svg>
  );
}

function CaptureGate({ ev, btn, accentCol, accent, deep, onDone, onSkip }: { ev: any; btn: string; accentCol: string; accent: string; deep: string; onDone: (phone: string) => void; onSkip: () => void }) {
  const [ph, setPh] = useState('');
  const redirectRef = useRef<string | null>(null);
  useEffect(() => { fetch(`${BACKEND}/plink?capcfg=${ev.id}`).then((r) => r.json()).then((d) => { redirectRef.current = d.redirect || null; }).catch(() => {}); }, [ev.id]);
  ...
  const submit = async () => {
    ...
    setBusy(true); setErr('');
    try {
      const r = await fetch(`${BACKEND}/leads`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ eventId: ev.id, phone: `+1${digits.slice(-10)}`, welcome: true }) }).then((x) => x.json());
      if (!r.ok) { setErr(r.reason === 'landline' ? "That's a landline — use your cell" : 'That number doesn\u2019t look right'); return; }
      trackLead(ev.id, ev.name);
      if (redirectRef.current) { setTimeout(() => { window.location.href = redirectRef.current!; }, 150); return; }
      onDone(ph);
    } catch { if (redirectRef.current) { window.location.href = redirectRef.current; return; } onDone(ph); }
    ...
    <div style={{ minHeight: '100vh', fontFamily: F, background: '#000' }}>
      <div style={{ maxWidth: 520, margin: '0 auto', padding: '20px 18px 60px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, margin: '0 0 10px' }}>
          <GozaStar color={accentCol} size={22} />
          <span style={{ color: accentCol, fontFamily: FL, fontSize: 11.5, letterSpacing: 2.6, fontWeight: 700, textTransform: 'uppercase' }}>Members only</span>
        </div>
        ...
        <div style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 18, padding: 22, marginTop: 18 }}>
          <p style={{ color: '#fff', fontSize: 18, fontWeight: 800, margin: '0 0 4px' }}>{ev.dropHeadline || 'Sign up to unlock tickets 🔓'}</p>
          <p style={{ color: '#9aa0aa', fontSize: 13.5, margin: '0 0 16px', lineHeight: 1.5 }}>Drop your number — takes 5 seconds, tickets open right after.</p>
          <input
            value={ph} onChange={(e) => { setPh(e.target.value); setErr(''); }} inputMode="tel" type="tel" placeholder="Your number"
            onKeyDown={(e) => { if (e.key === 'Enter') submit(); }}
            style={{ width: '100%', boxSizing: 'border-box', background: '#0e0e12', border: `1px solid ${err ? '#ff6b6b' : 'rgba(255,255,255,0.15)'}`, borderRadius: 12, padding: '15px 16px', color: '#fff', fontSize: 16, marginBottom: 12, outline: 'none', fontFamily: F }}
          />
          {err && <p style={{ color: '#ff8585', fontSize: 13, margin: '0 0 12px' }}>{err}</p>}
          <button onClick={submit} disabled={busy}
            style={{ width: '100%', background: btn, color: '#fff', border: 'none', borderRadius: 12, padding: '15px 0', fontSize: 16, fontWeight: 800, cursor: busy ? 'default' : 'pointer', opacity: busy ? 0.6 : 1, fontFamily: F }}>
            {busy ? 'One sec…' : 'UNLOCK TICKETS →'}
          </button>
        ...
        <button onClick={() => { if (redirectRef.current) { window.location.href = redirectRef.current; return; } onSkip(); }}
          style={{ display: 'block', margin: '18px auto 0', background: 'none', border: 'none', color: '#8a8f98', fontSize: 13, fontWeight: 600, cursor: 'pointer', textDecoration: 'underline', fontFamily: F }}>
          Skip — just show me tickets
        </button>
      </div>
    </div>
  );
...
/* ---------- VIP SEAT PICKER (selection only — checkout runs through the one main flow) ---------- */
function SeatPicker({ eventId, accent, tierName, selected, onChange }: { eventId: string; accent: string; tierName: string; selected: string[]; onChange: (ids: string[], labels: string[], faceTotal: number) => void }) {
  const [data, setData] = useState<any>(null);
  const load = () => fetch(`${BACKEND}/seats?eventId=${eventId}`).then((r) => r.json()).then((d) => { if (d.hasSeats) setData(d); }).catch(() => {});
  useEffect(() => { load(); const iv = setInterval(load, 30000); return () => clearInterval(iv); }, [eventId]);
  if (!data) return <p style={{ color: '#8a8f98', fontSize: 13, margin: '0 0 14px' }}>Loading seats…</p>;

  const allSeats: any[] = data.rows.flatMap((r: any) => r.seats.map((x: any) => ({ ...x, row: r.label })));
  const emit = (ids: string[]) => {
    const chosen = ids.map((id) => allSeats.find((x) => x.id === id)).filter(Boolean);
    onChange(ids, chosen.map((x: any) => `${x.row}${x.n}`), chosen.reduce((a: number, x: any) => a + (x.price || 0), 0));
  };
  const toggle = (seat: any) => {
    if (seat.status !== 'open') return;
    const next = selected.includes(seat.id) ? selected.filter((x) => x !== seat.id) : (selected.length >= 10 ? selected : [...selected, seat.id]);
    emit(next);
  };
  const Dot = ({ seat }: { seat: any }) => {
    const isSel = selected.includes(seat.id);
    const bg = seat.status === 'sold' || seat.status === 'held' ? '#2a2a31' : isSel ? accent : 'transparent';
    const border = seat.status === 'open' ? `1.5px solid ${isSel ? accent : 'rgba(255,255,255,0.45)'}` : '1.5px solid transparent';
    return <button onClick={() => toggle(seat)} aria-label={`Seat ${seat.row}${seat.n}`} style={{ width: 21, height: 21, borderRadius: '50%', background: bg, border, cursor: seat.status === 'open' ? 'pointer' : 'default', padding: 0, flexShrink: 0, transition: 'background 0.12s' }} />;
  };
  return (
    <div style={{ marginBottom: 16 }}>
      <p style={{ color: '#fff', fontSize: 14, fontWeight: 800, margin: '0 0 3px' }}>Pick your {tierName} seats</p>
      <p style={{ color: '#8a8f98', fontSize: 12, margin: '0 0 12px' }}>Tap open seats — they lock in when you pay.</p>
      <div style={{ background: '#17171d', borderRadius: 8, textAlign: 'center', color: '#8a8f98', fontSize: 11, fontWeight: 800, letterSpacing: 4, padding: '6px 0', marginBottom: 12 }}>STAGE</div>
          ...
          {data.rows.map((row: any) => {
            ...
            return (
              <div key={row.label} style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 6 }}>
                <span style={{ color: '#8a8f98', fontSize: 11.5, fontWeight: 800, width: 16, flexShrink: 0 }}>{row.label}</span>
                {C.length > 0 && <span style={{ display: 'flex', gap: 4 }}>{C.map((x: any) => <Dot key={x.id} seat={{ ...x, row: row.label }} />)}</span>}
                {L.length > 0 && (<><span style={{ display: 'flex', gap: 4 }}>{L.map((x: any) => <Dot key={x.id} seat={{ ...x, row: row.label }} />)}</span><span style={{ width: 22, flexShrink: 0 }} /><span style={{ display: 'flex', gap: 4 }}>{R.map((x: any) => <Dot key={x.id} seat={{ ...x, row: row.label }} />)}</span></>)}
              </div>
            );
          })}
        </div>
      ...
      <div style={{ display: 'flex', gap: 14, marginTop: 10 }}>
        <span style={{ color: '#8a8f98', fontSize: 11.5, display: 'flex', alignItems: 'center', gap: 5 }}><span style={{ width: 12, height: 12, borderRadius: '50%', border: '1.5px solid rgba(255,255,255,0.45)' }} /> Open</span>
        <span style={{ color: '#8a8f98', fontSize: 11.5, display: 'flex', alignItems: 'center', gap: 5 }}><span style={{ width: 12, height: 12, borderRadius: '50%', background: accent }} /> Yours</span>
        <span style={{ color: '#8a8f98', fontSize: 11.5, display: 'flex', alignItems: 'center', gap: 5 }}><span style={{ width: 12, height: 12, borderRadius: '50%', background: '#2a2a31' }} /> Taken</span>
      </div>
    </div>
  );
}

/* seatTierIds loader: knows which ticket types are seat tiers (drives the unified flow) */
function useSeatTiers(eventId: string, setSeatTierIds: (ids: string[]) => void) {
  useEffect(() => {
    fetch(`${BACKEND}/seats?eventId=${eventId}`).then((r) => r.json()).then((d) => { if (d.hasSeats && d.seatTierIds) setSeatTierIds(d.seatTierIds); }).catch(() => {});
  }, [eventId, setSeatTierIds]);
}
...
function EventMap({ location, accent = '#2a0f18', accentCol = '#c25b6e' }: { location: string; accent?: string; accentCol?: string }) {
  ...
  return (
    <div style={{ marginBottom: 30 }}>
      <div style={{ position: 'relative', height: 236, borderRadius: 18, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.10)', boxShadow: '0 16px 40px rgba(0,0,0,0.42)', background: '#0B0D12' }}>
        {/* the raw OSM tiles are daylight-bright; darken + desaturate them and tint
            with the event's own accent so the map belongs to the page */}
        <iframe title="Event location" src={src} loading="lazy"
          style={{ width: '100%', height: 'calc(100% + 22px)', border: 0, display: 'block',
                   filter: 'grayscale(0.85) brightness(0.62) contrast(1.12)' }} />
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: `linear-gradient(180deg, ${accent}22 0%, transparent 45%, rgba(0,0,0,0.55) 100%)`, mixBlendMode: 'multiply' }} />
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.10)' }} />
        <a href={mapsUrl} target="_blank" rel="noreferrer"
          style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'flex-end', justifyContent: 'flex-end', padding: 14, textDecoration: 'none' }}>
          <span style={{ background: accentCol, color: '#fff', fontFamily: FL, fontSize: 12, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', padding: '11px 18px', borderRadius: 999, boxShadow: '0 8px 20px rgba(0,0,0,0.45), inset 0 1px 0 rgba(255,255,255,0.3)' }}>Get directions ↗</span>
        </a>
      </div>
      <p style={{ color: '#6f747d', fontSize: 11, margin: '8px 2px 0', letterSpacing: 0.2 }}>
        Map data © <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noreferrer" style={{ color: '#8a8f98' }}>OpenStreetMap</a> contributors
      </p>
    </div>
  );
}
...
function Row({ l, v, dim, big, green }: { l: string; v: string; dim?: boolean; big?: boolean; green?: boolean }) {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', alignItems: 'baseline' }}>
      <span style={{ color, fontFamily: big ? FL : F, fontSize: big ? 15 : 14.5, fontWeight: big ? 700 : 400, letterSpacing: big ? 0.8 : 0, textTransform: big ? 'uppercase' : 'none' }}>{l}</span>
      <span style={{ color, fontFamily: big ? FD : F, fontSize: big ? 23 : 14.5, fontWeight: big ? 400 : 500 }}>{v}</span>
    </div>
  );
}

const F = "'Saira Condensed','Arial Narrow',Helvetica,Arial,sans-serif";
const FD = "'Archivo Black',Impact,Haettenschweiler,sans-serif";   // money + headlines
const FL = "'Chakra Petch','Trebuchet MS',sans-serif";             // labels + buttons
const SH: React.CSSProperties = { color: '#fff', fontFamily: FD, fontSize: 22, margin: '4px 0 16px', letterSpacing: 0, textTransform: 'uppercase' };
const SHEETH: React.CSSProperties = { color: '#fff', fontFamily: FD, fontSize: 24, margin: '4px 0 8px', textTransform: 'uppercase' };
const SHEETSUB: React.CSSProperties = { color: '#8a8f98', fontSize: 14.5, margin: '0 0 20px', lineHeight: 1.5 };
const QBTN: React.CSSProperties = { width: 54, height: 54, borderRadius: 16, background: 'linear-gradient(180deg,#26262c 0%,#141418 100%)', color: '#fff', border: '1px solid rgba(255,255,255,0.10)', fontSize: 26, cursor: 'pointer', boxShadow: 'inset 0 1px 0 rgba(255,255,255,.10)' };
const INPUT: React.CSSProperties = { width: '100%', height: 56, background: '#0f0f13', color: '#fff', border: '1px solid rgba(255,255,255,0.14)', borderRadius: 12, padding: '0 16px', fontFamily: F, fontSize: 17, outline: 'none', boxSizing: 'border-box', marginBottom: 12 };
const PRIMARY: React.CSSProperties = { width: '100%', background: '#f2f2f2', color: '#000', border: 'none', borderRadius: 999, padding: '17px 0', fontFamily: FL, fontSize: 16, fontWeight: 700, letterSpacing: 0.8, textTransform: 'uppercase', cursor: 'pointer', marginTop: 6, boxShadow: '0 8px 20px rgba(0,0,0,.3), inset 0 1px 0 rgba(255,255,255,.55)' };
