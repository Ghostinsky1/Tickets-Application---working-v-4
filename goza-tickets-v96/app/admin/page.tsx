...
function BarMenu({ adminKey, eventId }: { adminKey: string; eventId: string }) {
  ...
  useEffect(() => {
    if (!eventId) return;
    fetch(`${BACKEND}/admin`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-admin-key': adminKey }, body: JSON.stringify({ action: 'event_detail', eventId }) })
      .then((r) => r.json())
      .then((d) => {
        const ev = d?.event || d;
        if (ev?.bar_tax_percent !== undefined) setTaxPct(String(ev.bar_tax_percent ?? ''));
        if (ev?.bar_tax_included !== undefined) setTaxIncluded(!!ev.bar_tax_included);
      })
      .catch(() => {});
  }, [eventId, adminKey]);
  const saveTax = async () => {
    const r = await fetch(`${BACKEND}/admin`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-admin-key': adminKey },
      body: JSON.stringify({ action: 'update_event', eventId, patch: { bar_tax_percent: Number(taxPct) || 0, bar_tax_included: taxIncluded } }) }).then((x) => x.json());
    setMsg(r?.error ? r.error : `Bar tax saved: ${Number(taxPct) || 0}%${taxIncluded ? ' (included in prices)' : ' added at the till'}`);
  };
