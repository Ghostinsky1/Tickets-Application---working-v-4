...
function CreditChoice({ credit, session, onTakeCredit }: { credit: any; session: string; onTakeCredit: () => void }) {
  ...
  const refund = async () => {
    ...
    try {
      ...
      // confirmation email, so they know the amount and when it lands
      fetch(`${BACKEND}/refundmail`, { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ session, code: credit.code }) }).catch(() => {});
      setDone('refund');
    } catch { setErr('Could not reach us just now - try again'); }
    finally { setBusy(''); }
  };
    ...
    return (
      <div style={{ background: '#0E1116', border: '1px solid #232936', borderRadius: 18, padding: 22, marginBottom: 22 }}>
        ...
        <p style={{ color: '#9AA5B5', fontSize: 14.5, lineHeight: 1.55, margin: 0 }}>
          We&apos;ll send it back to the card you paid with. Refunds go out at the start of the week, then your bank takes 7&ndash;15 days. Confirmation is in your email.
        </p>
      </div>
    );
  }
...
/* ---------- MOVED OR CALLED OFF: what this buyer can do ---------- */
function ShowChanged({ session, onDone }: { session: string; onDone: () => void }) {
  ...
  const load = useCallback(() => {
    fetch(`${BACKEND}/ticketchoice`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'options', session }) })
      .then((r) => r.json()).then((d) => { if (d.ok) setRows(d.orders || []); }).catch(() => {});
  }, [session]);
  useEffect(() => { load(); }, [load]);

  const choose = async (orderId: string, want: 'credit' | 'refund') => {
    setBusy(true); setMsg('');
    try {
      const r = await fetch(`${BACKEND}/ticketchoice`, { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'choose', session, orderId, want }) }).then((x) => x.json());
      if (!r.ok) { setMsg(r.error || 'Could not do that'); return; }
      if (want === 'credit') sfxCoin(); else sfxConfirm();
      setMsg(r.message || '');
      setOpen(''); load(); onDone();
    } catch { setMsg('Could not reach us just now'); }
    finally { setBusy(false); }
  };
  ...
  return (
    <>
      {rows.map((r) => (
        <div key={r.orderId} style={{ background: '#0E1116', border: `1px solid ${r.state === 'cancelled' ? '#4a2630' : '#4a3a26'}`, borderRadius: 18, padding: 20, marginBottom: 18 }}>
          <p style={{ margin: 0, fontFamily: "'Chakra Petch','Trebuchet MS',sans-serif", fontWeight: 700, fontSize: 10, letterSpacing: 2.2, textTransform: 'uppercase', color: r.state === 'cancelled' ? '#FF8585' : '#FFB86B' }}>
            {r.state === 'cancelled' ? 'Called off' : 'New date'}
          </p>
          <p style={{ margin: '7px 0 6px', fontFamily: "'Archivo Black',Impact,sans-serif", fontSize: 20, color: '#fff', textTransform: 'uppercase', lineHeight: 1.1 }}>{r.event}</p>

          {r.state === 'postponed' ? (
            <p style={{ margin: '0 0 14px', color: '#9AA5B5', fontSize: 14.5, lineHeight: 1.55 }}>
              We&apos;re coming back {r.newDate ? <strong style={{ color: '#fff' }}>{r.newDate}</strong> : 'soon — date landing shortly'}. <strong style={{ color: '#fff' }}>Your ticket still works</strong>, nothing to do.
            </p>
          ) : (
            <p style={{ margin: '0 0 14px', color: '#9AA5B5', fontSize: 14.5, lineHeight: 1.55 }}>
              This one isn&apos;t happening. You paid {r.paid} — take it as credit or get it back.
            </p>
          )}

          {r.decided ? (
            <p style={{ margin: 0, color: '#4ADE80', fontSize: 14 }}>
              {r.decided === 'credit' ? 'Credit is on your account.' : 'Refund is on the way.'}
            </p>
          ) : !r.canChoose ? null : open === r.orderId || r.state === 'cancelled' ? (
            <div style={{ display: 'grid', gap: 9 }}>
              <button onClick={() => choose(r.orderId, 'credit')} disabled={busy}
                style={{ background: '#1140F0', color: '#fff', border: 'none', borderRadius: 14, padding: '16px 0', fontFamily: "'Archivo Black',Impact,sans-serif", fontSize: 17, textTransform: 'uppercase', cursor: 'pointer', boxShadow: '0 8px 22px rgba(17,64,240,.4)' }}>
                {r.paid} in credits
              </button>
              <button onClick={() => choose(r.orderId, 'refund')} disabled={busy}
                style={{ background: 'none', color: '#9AA5B5', border: '1px solid #232936', borderRadius: 14, padding: '14px 0', fontSize: 14.5, cursor: 'pointer' }}>
                Just give me my money back
              </button>
              <p style={{ margin: '2px 0 0', color: '#6d7787', fontSize: 12, lineHeight: 1.5, textAlign: 'center' }}>
                Credits land now and never expire. Refunds go out Mondays, then 7&ndash;15 days at your bank.
              </p>
            </div>
          ) : (
            <button onClick={() => setOpen(r.orderId)}
              style={{ background: 'none', border: '1px solid #232936', color: '#9AA5B5', borderRadius: 999, padding: '12px 18px', fontSize: 13.5, cursor: 'pointer' }}>
              Can&apos;t make the new date?
            </button>
          )}

          {msg && open === '' && <p style={{ margin: '12px 0 0', color: '#4ADE80', fontSize: 13.5, lineHeight: 1.5 }}>{msg}</p>}
        </div>
      ))}
    </>
  );
...
export default function Account() {
  ...
  useEffect(() => {
    ...
    fetch(`${BACKEND}/join`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'my_code', session: sessionToken }) })
      .then((r) => r.json())
      .then((d) => { if (d?.hasCode) setPromoCode(d.code); })
      .catch(() => {});
  }, [sessionToken]);
      ...
      <div style={{ maxWidth: 560, margin: '0 auto', padding: '28px 18px 60px' }}>
        ...
        {phase === 'portal' && data && (
          <div>
            ...
            {promoCode ? (
              <a href={`/p?c=${promoCode}`} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, textDecoration: 'none',
                background: 'rgba(17,64,240,0.12)', border: '1px solid rgba(17,64,240,0.45)', borderRadius: 14, padding: '14px 16px', marginBottom: 14 }}>
                <span>
                  <span style={{ display: 'block', color: '#8FB4FF', fontFamily: "'Chakra Petch','Trebuchet MS',sans-serif", fontWeight: 700, fontSize: 10, letterSpacing: 2, textTransform: 'uppercase' }}>Promoter</span>
                  <span style={{ display: 'block', color: '#fff', fontSize: 14.5, marginTop: 3 }}>See your clicks, sales and earnings</span>
                </span>
                <span style={{ color: '#fff', fontSize: 20 }}>&rarr;</span>
              </a>
            ) : (
              <a href="/join" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, textDecoration: 'none',
                background: 'rgba(17,64,240,0.08)', border: '1px solid rgba(17,64,240,0.3)', borderRadius: 14, padding: '14px 16px', marginBottom: 14 }}>
                <span>
                  <span style={{ display: 'block', color: '#8FB4FF', fontFamily: "'Chakra Petch','Trebuchet MS',sans-serif", fontWeight: 700, fontSize: 10, letterSpacing: 2, textTransform: 'uppercase' }}>Make money with us</span>
                  <span style={{ display: 'block', color: '#fff', fontSize: 14.5, marginTop: 3 }}>$2 a ticket for bringing your people</span>
                </span>
                <span style={{ color: '#fff', fontSize: 20 }}>&rarr;</span>
              </a>
            )}

            <a href="/refer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, textDecoration: 'none',
              background: 'rgba(74,222,128,0.08)', border: '1px solid rgba(74,222,128,0.35)', borderRadius: 14, padding: '14px 16px', marginBottom: 14 }}>
              <span>
                <span style={{ display: 'block', color: '#4ADE80', fontFamily: "'Chakra Petch','Trebuchet MS',sans-serif", fontWeight: 700, fontSize: 10, letterSpacing: 2, textTransform: 'uppercase' }}>Bring a friend</span>
                <span style={{ display: 'block', color: '#fff', fontSize: 14.5, marginTop: 3 }}>You both get $5 in credits</span>
              </span>
              <span style={{ color: '#fff', fontSize: 20 }}>&rarr;</span>
            </a>
            ...
            {sessionToken && <ShowChanged session={sessionToken} onDone={() => loadCredits(sessionToken)} />}
