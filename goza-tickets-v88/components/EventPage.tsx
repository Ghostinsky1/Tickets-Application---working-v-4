...
export default function EventPage({ eventId = DEFAULT_EVENT_ID }: { eventId?: string }) {
  ...
  const sendCode = async () => {
    ...
    try {
      ...
      if (r.verified || r.skip) { try { localStorage.setItem('gz_vphone', phoneDigits(phone)); } catch {} setStep('contact'); return; }
      if (r.error) { setOtpErr(r.error); if (step !== 'code') setStep('code'); return; }
      setResendIn(30);
      if (step !== 'code') setStep('code');
    } catch { setStep('contact'); } // network fail: never block
  ...
  const checkCode = async (code: string) => {
    ...
    try {
      ...
      if (r.verified) { try { localStorage.setItem('gz_vphone', phoneDigits(phone)); } catch {} setStep('contact'); return; }
          ...
          <div style={{ width: '100%', maxWidth: 520, background: '#0b0b0b', borderRadius: '22px 22px 0 0', maxHeight: '92vh', display: 'flex', flexDirection: 'column', overscrollBehavior: 'contain' }} onClick={(e) => e.stopPropagation()}>
            ...
            {step === 'phone' && (
              <>
                ...
                <button onClick={() => { try { fetch(`${BACKEND}/leads`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ eventId, phone: `+1${phoneDigits(phone)}` }), keepalive: true }); } catch {} let v = ''; try { v = localStorage.getItem('gz_vphone') || ''; } catch {} if (v === phoneDigits(phone)) { setStep('contact'); } else { setOtpCode(''); sendCode(); } }} disabled={!phoneOk || otpBusy} style={{ ...PRIMARY, background: btn, color: '#fff', opacity: phoneOk && !otpBusy ? 1 : 0.45 }}>{otpBusy ? 'Sending code…' : 'Continue'}</button>
              </>
            )}
