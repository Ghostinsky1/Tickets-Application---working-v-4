          ...
          {details.length > 0 && (
            <>
              ...
              <div style={{ background: CARD, backdropFilter: 'blur(8px)', borderRadius: 16, padding: 20, marginBottom: 28, border: '1px solid rgba(255,255,255,0.07)' }}>
                ...
                <div style={{ position: 'relative', maxHeight: detailsOpen ? 'none' : 108, overflow: 'hidden' }}>
                  ...
                  {!detailsOpen && details.length > 2 && (
                    <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, height: 52, background: 'linear-gradient(180deg, rgba(10,8,12,0) 0%, rgba(10,8,12,0.92) 85%)', pointerEvents: 'none' }} />
                  )}
                </div>
                {details.length > 2 && (
                  <button onClick={() => setDetailsOpen(!detailsOpen)}
                    style={{ background: 'none', border: 'none', color: accentCol, fontSize: 14.5, fontWeight: 700, cursor: 'pointer', padding: '12px 0 0', display: 'flex', alignItems: 'center', gap: 6 }}>
