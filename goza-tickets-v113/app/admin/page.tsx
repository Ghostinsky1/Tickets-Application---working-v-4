...
function Panel({ adminKey, onLogout }: { adminKey: string; onLogout: () => void }) {
  ...
  return (
    <div className="gz-admin" style={{ display: 'flex', minHeight: '100vh', background: '#000', backgroundAttachment: 'fixed', fontFamily: BODY }}>
      <style>{`
        .gz-admin { align-items: stretch; }
        .gz-side { width: 220px; flex-shrink: 0; }
        .gz-main { flex: 1; min-width: 0; padding: 26px 30px 70px; }
        .gz-main > * { max-width: 1180px; }
        /* wide screens: stop the dead column on the right */
        @media (min-width: 1400px) { .gz-main > * { max-width: 1320px; } }
        /* phones: sidebar becomes a scrolling tab strip */
        @media (max-width: 860px) {
          .gz-admin { display: block; }
          .gz-side { width: 100%; border-right: none; border-bottom: 1px solid #232936; position: sticky; top: 0; z-index: 30; }
          .gz-side-head { display: flex; align-items: center; justify-content: space-between; padding: 12px 14px !important; text-align: left !important; }
          .gz-nav { display: flex !important; overflow-x: auto; gap: 6px; padding: 8px 10px !important; -webkit-overflow-scrolling: touch; scrollbar-width: none; }
          .gz-nav::-webkit-scrollbar { display: none; }
          .gz-nav button { width: auto !important; margin: 0 !important; white-space: nowrap; padding: 10px 14px !important; font-size: 13.5px !important; }
          .gz-main { padding: 16px 14px 60px; }
          .gz-main input, .gz-main select, .gz-main textarea { font-size: 16px !important; }
        }
      `}</style>
      <aside className="gz-side" style={{ borderRight: `1px solid ${T.line}`, display: 'flex', flexDirection: 'column', background: 'rgba(8,8,12,0.55)', backdropFilter: 'blur(8px)' }}>
        <div className="gz-side-head" style={{ padding: '22px 16px 18px', borderBottom: `1px solid ${T.line}`, textAlign: 'center' }}>
        ...
        <nav className="gz-nav" style={{ flex: 1, padding: '14px 0' }}>
          {NAV.map((n) => (
        ...
        <>
          ...
          {rows && (
            <>
              ...
              {filtered.map((r) => (
                <div key={r.id} style={{ background: T.card, border: `1px solid ${T.line}`, borderRadius: 12, padding: '13px 16px', marginBottom: 9 }}>
...
function Events({ adminKey, events, eventId, setEventId, refresh }: any) {
  ...
  return (
    <>
      ...
      {/* create */}
      <div style={{ background: T.card, border: `1px solid ${T.line}`, padding: 20, marginBottom: 26 }}>
...
function EventMode({ adminKey, eventId }: { adminKey: string; eventId: string }) {
  ...
  const act = async (action: string, extra: any = {}) => {
    if (action === 'cancel' || action === 'postpone') {
      // show the actual text, who gets it and what it costs, before anything fires
      const pv = await call({ action: 'preview', kind: action === 'cancel' ? 'cancelled' : 'postponed', newDate });
      const tell = window.confirm([
        action === 'cancel' ? 'CANCEL THIS EVENT?' : `POSTPONE TO ${newDate}?`,
        ``,
        `${pv?.paidOrders ?? 0} paid buyer(s) get the credit-or-refund choice.`,
        ``,
        `TELL THEM NOW?`,
        `OK  = send the text + email below`,
        `Cancel = change the status quietly, you tell them yourself`,
        ``,
        `Text (${pv?.texts ?? 0} people, ${pv?.cost ?? '$0.00'}):`,
        `"${pv?.sms ?? ''}"`,
        ``,
        `Email: ${pv?.emails ?? 0} people`,
        pv?.optedOut ? `${pv.optedOut} opted out of texts - they still get the email.` : ``,
      ].filter(Boolean).join('\n'));
      setBusy(true); setMsg('');
      const r = await call({ action, refundMode: st?.refundMode || 'manual', newDate, notify: tell });
      setBusy(false);
