  ...
  return (
    <div style={{
      ...
      <p style={{ margin: 0, fontFamily: "'Chakra Petch','Trebuchet MS',sans-serif", fontSize: 10.5, letterSpacing: 2.6, fontWeight: 700, color: '#8FB4FF', textTransform: 'uppercase' }}>
        Goza credit{c?.fromEvent ? ` from ${c.fromEvent}` : ''}
      </p>
      ...
      <p style={{ margin: '0 0 16px', color: '#9AA5B5', fontSize: 13.5 }}>
        Ready to spend at Plaza Garibaldi and El Palenque events{c?.expiresAt ? ` - good through ${new Date(c.expiresAt).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}` : ''}
      </p>
    ...
    <div style={{ background: 'linear-gradient(160deg,#0B1B4F 0%,#0A0D16 70%)', border: '1px solid #22304f', borderRadius: 18, padding: '24px 20px', marginBottom: 22 }}>
      ...
      <button onClick={take} onMouseEnter={sfxTick} disabled={!!busy}
        ...
        <span style={{ display: 'block', fontFamily: "'Chakra Petch','Trebuchet MS',sans-serif", fontSize: 12.5, letterSpacing: 1, textTransform: 'uppercase', color: '#C9D6FF', marginTop: 6 }}>
          20% bonus included &middot; use it on any Goza night
        </span>
      </button>
...
function Reload({ session, balance, onDone }: { session: string; balance: string; onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    fetch(`${BACKEND}/wallet`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'topup_confirm', session, stripeSession: sid }) })
      .then((r) => r.json())
      .then((d) => {
  ...
  const go = async (cents: number) => {
    ...
    try {
      const r = await fetch(`${BACKEND}/wallet`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'topup_start', session, amountCents: cents }) }).then((x) => x.json());
  ...
  if (!open) {
    return (
      <button onClick={() => { setOpen(true); sfxClick(); }}
        ...
        <span style={{ display: 'block', fontFamily: "'Chakra Petch','Trebuchet MS',sans-serif", fontSize: 10.5, letterSpacing: 2.4, fontWeight: 700, color: '#8FB4FF', textTransform: 'uppercase', marginBottom: 5 }}>Add credit</span>
  ...
  return (
    <div style={{ background: 'linear-gradient(160deg,#0B1B4F 0%,#0A0D16 70%)', border: '1px solid #22304f', borderRadius: 18, padding: '20px 18px', marginBottom: 22 }}>
      <p style={{ margin: 0, fontFamily: "'Chakra Petch','Trebuchet MS',sans-serif", fontSize: 10.5, letterSpacing: 2.4, fontWeight: 700, color: '#8FB4FF', textTransform: 'uppercase' }}>Add credit</p>
      ...
      <p style={{ color: '#6d7787', fontSize: 11.5, lineHeight: 1.5, margin: 0 }}>
        Credit works at our Plaza Garibaldi and El Palenque events &mdash; not at outside venues. It can&apos;t be cashed out, and your balance stays on your account between events.
      </p>
