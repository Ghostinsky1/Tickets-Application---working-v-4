...
export default function Bar() {
  ...
  const charge = async () => {
    ...
    try {
      ...
      setDone({ charged: r.charged, remaining: r.remaining, name: guest.name });
    ...
    <div style={{ minHeight: '100vh', background: C.bg, color: '#fff', fontFamily: F, paddingBottom: totalCents > 0 ? 190 : 24 }}>
      ...
      {done && (
        <div style={{ margin: 16, background: 'rgba(74,222,128,.10)', border: `1px solid ${C.green}`, borderRadius: 14, padding: 16, textAlign: 'center' }}>
          ...
          <p style={{ color: '#cfd6e2', fontSize: 14, margin: '6px 0 0' }}>{done.name} · {done.remaining} left on their balance</p>
        </div>
      )}
          ...
          {guest ? (
            <>
              <div style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 12, padding: '11px 13px', marginBottom: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span style={{ color: guest.balanceCents >= totalCents ? C.green : C.bad, fontFamily: FD, fontSize: 17 }}>{guest.balance}</span>
              </div>
              <button onClick={charge} disabled={busy || guest.balanceCents < totalCents}
                style={{ width: '100%', background: guest.balanceCents < totalCents ? '#33383f' : C.green, color: guest.balanceCents < totalCents ? C.muted : '#04210f', border: 'none', borderRadius: 14, padding: '18px 0', fontFamily: FD, fontSize: 19, textTransform: 'uppercase', cursor: 'pointer' }}>
                {busy ? 'Charging…' : guest.balanceCents < totalCents ? 'Not enough balance' : `Charge ${money(totalCents)}`}
              </button>
