...
export default function EventPage({ eventId = DEFAULT_EVENT_ID }: { eventId?: string }) {
  ...
  const [captured, setCaptured] = useState(() => { try { return !!localStorage.getItem(`gz_cap_${eventId}`); } catch { return false; } });
  ...
  const pay = async () => {
    ...
    try {
      ...
      if (seated) {
        ...
        const d = await api('/seats', { method: 'POST', body: JSON.stringify({ action: 'checkout', eventId, seatIds: selSeats, holdKey: hk, buyerName: name.trim(), buyerEmail: email.trim(), buyerPhone: `+1${phoneDigits(phone)}` }) });
  ...
  if (ev.eventMode === 'capture' && !captured) {
    return <CaptureGate ev={ev} btn={btn} accentCol={accentCol} onDone={(nm, ph, em) => {
      setName(nm); setEmail(em); setPhone(ph);
      try { localStorage.setItem(`gz_cap_${ev.id}`, '1'); localStorage.setItem('gz_vphone', ph.replace(/\D/g, '')); } catch {}
      identifyUser(em, `+1${ph.replace(/\D/g, '')}`);
      setCaptured(true);
      setStep('cart');
    }} />;
  }
...
/* ---------- CAPTURE GATE (sign up -> unlock tickets, lead saved either way) ---------- */
function CaptureGate({ ev, btn, accentCol, onDone }: { ev: any; btn: string; accentCol: string; onDone: (name: string, phone: string, email: string) => void }) {
  ...
  const ok = nm.trim().length >= 2 && digits.length === 10 && /^[^@\s]+@[^@\s]+\.[^@\s]{2,}$/.test(em.trim());
  const go = async () => {
    setBusy(true); setErr('');
    try {
      const r = await fetch(`${BACKEND}/leads`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ eventId: ev.id, name: nm.trim(), phone: `+1${digits}`, email: em.trim().toLowerCase() }) }).then((x) => x.json());
      if (!r.ok) { setErr(r.reason === 'landline' ? "That's a landline — use your cell" : 'That number doesn\u2019t look right — double-check it'); return; }
      trackLead(ev.id, ev.name);
      onDone(nm.trim(), ph, em.trim());
    } catch { onDone(nm.trim(), ph, em.trim()); } // network hiccup: never block the sale
    finally { setBusy(false); }
  };
  return (
    <div style={{ minHeight: '100vh', background: '#050309', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '0 0 40px' }}>
      {ev.imageUrl && <img src={ev.imageUrl} alt={ev.name} style={{ width: '100%', maxWidth: 560, display: 'block' }} />}
      <div style={{ width: '100%', maxWidth: 560, boxSizing: 'border-box', padding: '22px 18px 0' }}>
        <p style={{ color: accentCol, fontSize: 12.5, fontWeight: 800, letterSpacing: 2, margin: '0 0 6px' }}>{ev.name.toUpperCase()}</p>
        <h1 style={{ color: '#fff', fontSize: 27, fontWeight: 800, lineHeight: 1.15, margin: '0 0 8px' }}>{ev.dropHeadline || 'Sign up to unlock tickets 🔓'}</h1>
        <p style={{ color: '#8a8f98', fontSize: 14, margin: '0 0 20px' }}>Takes 10 seconds. Tickets open right after.</p>
        <input value={nm} onChange={(e) => setNm(e.target.value)} placeholder="Full name" style={CAPINP} />
        <input value={ph} onChange={(e) => setPh(e.target.value)} placeholder="Phone" type="tel" inputMode="tel" style={CAPINP} />
        <input value={em} onChange={(e) => setEm(e.target.value)} placeholder="Email" type="email" style={CAPINP} />
        {err && <p style={{ color: '#ff6b6b', fontSize: 13, margin: '0 0 10px' }}>{err}</p>}
        <button onClick={go} disabled={!ok || busy}
          style={{ width: '100%', background: btn, color: '#fff', border: 'none', borderRadius: 13, padding: '16px 0', fontSize: 16, fontWeight: 800, cursor: 'pointer', opacity: ok && !busy ? 1 : 0.45 }}>
          {busy ? 'One sec…' : 'Sign me up →'}
        </button>
        <p style={{ color: '#6f747d', fontSize: 11.5, textAlign: 'center', margin: '12px 0 0' }}>By signing up you agree to get event texts. Reply STOP anytime.</p>
      </div>
    </div>
  );
}
const CAPINP: React.CSSProperties = { width: '100%', boxSizing: 'border-box', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.14)', borderRadius: 12, color: '#fff', fontSize: 16, padding: '15px 15px', marginBottom: 10, outline: 'none' };
