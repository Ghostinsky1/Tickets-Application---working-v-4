          ...
          <div>
            ...
            {data.orders.map((o: any) => (
              <div key={o.orderId} style={{ background: '#0c0c10', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 18, padding: 20, marginBottom: 18, opacity: o.isPast ? 0.6 : 1 }}>
                ...
                {o.tickets.map((t: any, i: number) => (
                  <div key={t.id} style={{ background: '#fff', borderRadius: 12, padding: 14, marginBottom: 10, display: 'flex', alignItems: 'center', gap: 14 }}>
                    <img src={`data:image/gif;base64,${t.qrGif}`} alt="QR" style={{ width: 84, height: 84, display: 'block' }} />
                    <div>
                      <p style={{ color: '#111', fontSize: 14, fontWeight: 800, margin: '0 0 2px' }}>{t.typeName}</p>
                      <p style={{ color: '#666', fontSize: 12, margin: 0 }}>Ticket {i + 1} of {o.tickets.length}</p>
                      {t.checkedIn && <p style={{ color: '#c25b6e', fontSize: 11.5, fontWeight: 700, margin: '4px 0 0' }}>✓ CHECKED IN</p>}
                    </div>
                  </div>
                ))}
