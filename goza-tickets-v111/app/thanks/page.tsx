...
export default function Thanks() {
  ...
  useEffect(() => {
    ...
    // credit-covered orders skip Stripe entirely and arrive as ?order=<id>
    const orderId = params.get('order');
    if (!sid && !orderId) { setTries(-1); return; }
    let stop = false;
    const poll = async (n: number) => {
      if (stop || n > 20) { setTries(n); return; }
      try {
        const d = orderId
          ? await api(`/receipt?order=${encodeURIComponent(orderId)}`)
          : await api(`/checkout?receipt=1&session=${encodeURIComponent(sid!)}`);
        if (d.found) {
          if (!stop) {
            setR(d);
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#000', fontFamily: F, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '32px 18px' }}>
      <div style={{ width: '100%', maxWidth: 430 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 22 }}>
          ...
          <h1 style={{ color: '#fff', fontFamily: "'Archivo Black',Impact,Haettenschweiler,sans-serif", fontSize: 30, margin: '0 0 6px', letterSpacing: 0, textTransform: 'uppercase' }}>You&apos;re in.</h1>
            ...
            <>
              ...
              <div style={{ display: 'flex', gap: 14, padding: 16, borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                ...
                <div style={{ minWidth: 0 }}>
                  <p style={{ color: '#fff', fontFamily: "'Archivo Black',Impact,Haettenschweiler,sans-serif", fontSize: 17, margin: '2px 0 6px', lineHeight: 1.2, textTransform: 'uppercase' }}>{ev?.name}</p>
              ...
              <div style={{ padding: '14px 16px' }}>
                ...
                {o.phone && !o.smsSent && (
                  <p style={{ color: '#FFB86B', fontSize: 13, lineHeight: 1.55, margin: '10px 0 0' }}>
                    No text? You may have replied STOP to us before, which blocks all our texts &mdash; including your tickets.
                    Text <strong>START</strong> to 314-876-6123 to turn them back on. Your tickets are in your email either way.
                  </p>
                )}
