    ...
    <div style={{ minHeight: '100vh', fontFamily: F, background: `radial-gradient(130% 100% at 50% 0%, ${accent} 0%, ${deep} 55%, #000 100%)`, backgroundAttachment: 'fixed' }}>
      ...
      {step && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 50, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={() => !submitting && setStep(null)}>
          <div style={{ width: '100%', maxWidth: 520, background: '#0b0b0b', borderRadius: '22px 22px 0 0', padding: '22px 22px calc(26px + env(safe-area-inset-bottom))', position: 'relative', maxHeight: '92vh', overflowY: 'auto' }} onClick={(e) => e.stopPropagation()}>
              ...
              <>
                ...
                {addon && (
                  <div onClick={() => setAddonOn((v) => !v)}
                    ...
                    <div style={{ flex: 1 }}>
                      <p style={{ color: '#fff', fontSize: 14.5, fontWeight: 600, margin: '0 0 2px', lineHeight: 1.3 }}>{addon.pitch}</p>
