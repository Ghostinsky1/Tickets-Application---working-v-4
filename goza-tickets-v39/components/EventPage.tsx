...
export default function EventPage({ eventId = DEFAULT_EVENT_ID }: { eventId?: string }) {
  ...
  const multi = cartEntries.length > 1;
  ...
  useEffect(() => { if (step === 'cart' && !multi) fetchQuote(ttId, qty, discount, addonOn); }, [step, ttId, qty, discount, addonOn, fetchQuote, multi]);
  useEffect(() => {
    if (step !== 'cart' || !multi) { setMultiQuote(null); return; }
    let dead = false;
    fetch(`${BACKEND}/cart`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'quote', eventId, items: cartEntries.map(([t, q]) => ({ ticketTypeId: t, quantity: q })) }) })
      .then((r) => r.json()).then((d) => { if (!dead && !d.error) setMultiQuote(d); }).catch(() => {});
    return () => { dead = true; };
  }, [step, multi, JSON.stringify(cartEntries), eventId]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const types = (info?.ticketTypes ?? []).filter((t: any) => !seatTierIds.includes(t.id));
  ...
  const pay = async () => {
    ...
    try {
      // MULTI-TYPE cart: one hosted-checkout session covering every type
      if (multi) {
        const d = await api('/cart', { method: 'POST', body: JSON.stringify({
          action: 'checkout', eventId,
          items: cartEntries.map(([t, q]) => ({ ticketTypeId: t, quantity: q })),
    ...
    <div style={{ minHeight: '100vh', fontFamily: F, background: `radial-gradient(130% 100% at 50% 0%, ${accent} 0%, ${deep} 55%, #000 100%)`, backgroundAttachment: 'fixed' }}>
      ...
      <div className="gz-shell">
        ...
        <div style={{ paddingBottom: 40 }}>
          ...
          <SeatMap eventId={eventId} accent={accentCol} btn={btn} onTiers={setSeatTierIds} />
              ...
              <>
                ...
                {types.length > 1 ? (
                  <div style={{ marginBottom: 16 }}>
                    {types.map((t) => {
                      const n = cart[t.id] || (t.id === ttId && !cartEntries.length ? qty : 0);
                      const soldOut = t.remaining <= 0;
                      return (
                        <div key={t.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '11px 0', borderBottom: '1px solid rgba(255,255,255,0.07)', opacity: soldOut ? 0.45 : 1 }}>
                          <div style={{ minWidth: 0 }}>
                            <p style={{ color: '#fff', fontSize: 14.5, fontWeight: 700, margin: 0 }}>{t.name}{soldOut ? ' — SOLD OUT' : ''}</p>
                            <p style={{ color: '#8a8f98', fontSize: 12.5, margin: '2px 0 0' }}>{money(t.price)}</p>
                          </div>
                          {!soldOut && (
                            <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
                              <button onClick={() => { const next = { ...cart, [t.id]: Math.max(0, n - 1) }; setCart(next); const e2 = Object.entries(next).filter(([, q]) => q > 0); if (e2.length) { setTtId(e2[0][0]); setQty(e2.reduce((a, [, q]) => a + q, 0)); } }} style={{ ...QBTN, width: 36, height: 36, fontSize: 19 }} aria-label="Fewer">−</button>
                              <span style={{ color: '#fff', fontSize: 19, fontWeight: 800, minWidth: 20, textAlign: 'center' }}>{n}</span>
                              <button onClick={() => { if (cartTotalQty >= maxPer) return; const next = { ...cart, [t.id]: Math.min(t.remaining, n + 1) }; setCart(next); const e2 = Object.entries(next).filter(([, q]) => q > 0); setTtId(e2[0][0]); setQty(e2.reduce((a, [, q]) => a + q, 0)); }} style={{ ...QBTN, width: 36, height: 36, fontSize: 19 }} aria-label="More">+</button>
                            </div>
                          )}
                        </div>
                      );
                    })}
                    {cartTotalQty >= maxPer && <p style={{ color: '#f0b64b', fontSize: 12, margin: '8px 0 0' }}>Max {maxPer} tickets per order</p>}
                  </div>
                ) : (
                ...
                {!multi && bump && !bumpActive && qty >= bump.triggerQty && (
                ...
                <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: 12, marginBottom: 8 }}>
                  {multi ? (
                    multiQuote ? (
                      <div>
                        {multiQuote.lines.map((l: any) => <Row key={l.ticketTypeId} l={`${l.name} × ${l.quantity}`} v={money(l.lineTotal)} />)}
                        <p style={{ color: '#6f747d', fontSize: 11.5, margin: '4px 0 0' }}>Line prices include fees</p>
                        <div style={{ height: 1, background: 'rgba(255,255,255,0.08)', margin: '8px 0' }} />
                        <Row l="Total" v={money(multiQuote.total)} big />
                      </div>
...
/* ---------- RESERVED SEATING (tap your seats, they hold for 8 min) ---------- */
function SeatMap({ eventId, accent, btn, onTiers }: { eventId: string; accent: string; btn: string; onTiers?: (ids: string[]) => void }) {
  const [data, setData] = useState<any>(null);
  const [sel, setSel] = useState<string[]>([]);
  const [stage, setStage] = useState<'pick' | 'contact' | 'holding'>('pick');
  const [nm, setNm] = useState(''); const [em, setEm] = useState(''); const [ph, setPh] = useState('');
  const [err, setErr] = useState(''); const [busy, setBusy] = useState(false);
  const holdKey = (() => { try { let k = localStorage.getItem('gz_seat_key'); if (!k) { k = Math.random().toString(36).slice(2) + Date.now().toString(36); localStorage.setItem('gz_seat_key', k); } return k; } catch { return 'anon'; } })();

  const load = () => fetch(`${BACKEND}/seats?eventId=${eventId}`).then((r) => r.json()).then((d) => { if (d.hasSeats) { setData(d); if (onTiers && d.seatTierIds) onTiers(d.seatTierIds); } }).catch(() => {});
  useEffect(() => { const t = setTimeout(load, 600); const iv = setInterval(load, 30000); return () => { clearTimeout(t); clearInterval(iv); }; }, [eventId]);
  if (!data) return null;

  const allSeats: any[] = data.rows.flatMap((r: any) => r.seats.map((x: any) => ({ ...x, row: r.label })));
  const priceOf = (id: string) => allSeats.find((x) => x.id === id)?.price ?? 0;
  const labelOf = (id: string) => { const x = allSeats.find((y) => y.id === id); return x ? `${x.row}${x.n}` : ''; };
  const total = sel.reduce((a, id) => a + (priceOf(id) || 0), 0);
  const toggle = (seat: any) => {
    if (seat.status !== 'open') return;
    setErr('');
    setSel((p) => p.includes(seat.id) ? p.filter((x) => x !== seat.id) : (p.length >= 10 ? p : [...p, seat.id]));
  };
  const holdAndContinue = async () => {
    setBusy(true); setErr('');
    try {
      const r = await fetch(`${BACKEND}/seats`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'hold', eventId, seatIds: sel, holdKey }) }).then((x) => x.json());
      if (!r.ok) { setErr(r.error || 'Those seats just got taken'); await load(); setSel([]); return; }
      setStage('contact');
    } catch (e: any) { setErr(e.message); }
    finally { setBusy(false); }
  };
  const paySeats = async () => {
    setBusy(true); setErr('');
    try {
      const digits = ph.replace(/\D/g, '');
      try { fetch(`${BACKEND}/leads`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ eventId, name: nm, phone: `+1${digits}`, email: em }), keepalive: true }); } catch {}
      const r = await fetch(`${BACKEND}/seats`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'checkout', eventId, seatIds: sel, holdKey, buyerName: nm, buyerEmail: em, buyerPhone: digits.length === 10 ? `+1${digits}` : '' }) }).then((x) => x.json());
  ...
  const Dot = ({ seat }: { seat: any }) => {
    const isSel = sel.includes(seat.id);
    ...
    return (
      <button onClick={() => toggle(seat)} aria-label={`Seat ${seat.row}${seat.n}`}
        style={{ width: 21, height: 21, borderRadius: '50%', background: bg, border, cursor: seat.status === 'open' ? 'pointer' : 'default', padding: 0, flexShrink: 0, transition: 'background 0.12s' }} />
    );
  };

  return (
    <div style={{ background: CARD, backdropFilter: 'blur(8px)', borderRadius: 16, padding: '18px 14px', marginBottom: 26, border: '1px solid rgba(255,255,255,0.07)' }}>
      <p style={{ color: '#fff', fontSize: 15.5, fontWeight: 800, margin: '0 0 3px', paddingLeft: 4 }}>🪑 Pick your seats</p>
      <p style={{ color: '#8a8f98', fontSize: 12.5, margin: '0 0 14px', paddingLeft: 4 }}>Tap open seats. They hold for {data.holdMinutes} min while you pay.</p>

      <div style={{ background: '#17171d', borderRadius: 8, textAlign: 'center', color: '#8a8f98', fontSize: 11, fontWeight: 800, letterSpacing: 4, padding: '7px 0', marginBottom: 14 }}>STAGE</div>
          ...
          {data.rows.map((row: any) => {
            ...
            return (
              <div key={row.label} style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 6 }}>
                ...
                {L.length > 0 && (
                  <>
                    <span style={{ display: 'flex', gap: 4 }}>{L.map((x: any) => <Dot key={x.id} seat={{ ...x, row: row.label }} />)}</span>
                    <span style={{ width: 22, flexShrink: 0 }} />
                    <span style={{ display: 'flex', gap: 4 }}>{R.map((x: any) => <Dot key={x.id} seat={{ ...x, row: row.label }} />)}</span>
                  </>
                )}
              </div>
            );
      ...
      <div style={{ display: 'flex', gap: 14, margin: '12px 0 0', paddingLeft: 4 }}>
      ...
      {err && <p style={{ color: '#ff6b6b', fontSize: 13, margin: '10px 4px 0' }}>{err}</p>}

      {stage === 'pick' && sel.length > 0 && (
        <button onClick={holdAndContinue} disabled={busy}
          style={{ width: '100%', marginTop: 14, background: btn, color: '#fff', border: 'none', borderRadius: 12, padding: '15px 0', fontSize: 15.5, fontWeight: 800, cursor: 'pointer', opacity: busy ? 0.55 : 1 }}>
          {busy ? 'Holding…' : `Continue — ${sel.map(labelOf).join(', ')} · $${total.toFixed(2)}`}
        </button>
      )}

      {stage === 'contact' && (
        <div style={{ marginTop: 14 }}>
          <p style={{ color: accent, fontSize: 12.5, fontWeight: 800, margin: '0 0 10px', paddingLeft: 4 }}>⏱ Seats {sel.map(labelOf).join(', ')} held for {data.holdMinutes} minutes</p>
          <input value={nm} onChange={(e) => setNm(e.target.value)} placeholder="Full name" style={SEATINP} />
          <input value={em} onChange={(e) => setEm(e.target.value)} placeholder="Email (tickets go here)" type="email" style={SEATINP} />
          <input value={ph} onChange={(e) => setPh(e.target.value)} placeholder="Phone (for your confirmation text)" type="tel" style={SEATINP} />
          <button onClick={paySeats} disabled={busy || nm.trim().length < 2 || !/^[^@\s]+@[^@\s]+\.[^@\s]{2,}$/.test(em)}
            style={{ width: '100%', background: btn, color: '#fff', border: 'none', borderRadius: 12, padding: '15px 0', fontSize: 15.5, fontWeight: 800, cursor: 'pointer', opacity: busy || nm.trim().length < 2 ? 0.55 : 1 }}>
            {busy ? 'One sec…' : `Pay $${total.toFixed(2)}`}
          </button>
        </div>
      )}
    </div>
...
const SEATINP: React.CSSProperties = { width: '100%', boxSizing: 'border-box', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.14)', borderRadius: 11, color: '#fff', fontSize: 15, padding: '13px 14px', marginBottom: 9, outline: 'none' };
