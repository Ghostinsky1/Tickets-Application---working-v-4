          ...
          <div style={{ width: '100%', maxWidth: 520, background: '#0b0b0b', borderRadius: '22px 22px 0 0', maxHeight: '92vh', display: 'flex', flexDirection: 'column', overscrollBehavior: 'contain' }} onClick={(e) => e.stopPropagation()}>
            ...
            {step === 'phone' && (
              <>
                ...
                <button onClick={() => { try { fetch(`${BACKEND}/leads`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ eventId, phone: `+1${phoneDigits(phone)}` }), keepalive: true }); } catch {} setStep('contact'); }} disabled={!phoneOk} style={{ ...PRIMARY, background: btn, color: '#fff', opacity: phoneOk ? 1 : 0.45 }}>Continue</button>
              </>
            )}
