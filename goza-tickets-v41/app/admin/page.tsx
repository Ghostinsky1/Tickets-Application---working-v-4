...
function SeatingManager({ adminKey, eventId }: { adminKey: string; eventId: string }) {
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${T.line}`, borderRadius: 14, padding: 18, marginTop: 18 }}>
      ...
      {hasSeats ? (
        <>
          <p style={{ color: T.dim, fontSize: 12.5, margin: '0 0 12px' }}>Live seat status — buyers pick these on the event page.</p>
          {counts.map((c: any) => (
            <div key={c.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              <span style={{ color: '#fff', fontSize: 13.5, fontWeight: 700 }}>Row {c.label}</span>
              <span style={{ color: T.dim, fontSize: 12.5 }}>{c.sold} sold · {c.open} open · {c.total} total</span>
            </div>
          ))}
