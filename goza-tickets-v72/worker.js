...
export default {
  async fetch(request, env) {
    ...
    // one-tap ticket access from the purchase SMS: /a/<token> -> signed-in account
    if (url.pathname.startsWith('/a/')) {
      const tok = url.pathname.slice(3).split('/')[0].trim();
      if (tok) return Response.redirect(`${url.origin}/account?token=${encodeURIComponent(tok)}`, 302);
    }
    ...
    if (url.pathname.startsWith('/t/')) {
      ...
      if (code) {
        try {
          ...
          // not an event code -> maybe a PROMOTER tracking link
          const pr = await fetch(`${backend}/plink?code=${encodeURIComponent(code)}`);
          if (pr.ok) {
            const pd = await pr.json();
            if (pd && pd.eventId) {
              return Response.redirect(`${url.origin}/e?id=${pd.eventId}${pd.promoterId ? `&p=${pd.promoterId}` : ''}`, 302);
            }
          }
