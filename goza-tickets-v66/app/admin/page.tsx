...
const T = {
  blue: '#1140F0', navy: '#04214D', ice: '#8FD3FF', iceDeep: '#6FB8F2',
  // kept under the old names so every existing usage inherits the new system
  red: '#1140F0', redBright: '#8FD3FF', chrome: '#8FD3FF',
  dim: '#A8B2C1', white: '#FFFFFF', text: '#D6DCE6',
  line: '#2A3040', line2: '#22262F',
  card: '#14171E', deep: '#0B0D12', black: '#08090D',
  good: '#6FE39B', warn: '#FFB86B', bad: '#FF7A7A',
};
...
const CHROME_BG = 'linear-gradient(180deg,#BFE6FF 0%,#6FB8F2 100%)';
const GLASS_BG = 'linear-gradient(180deg,#2A3040 0%,#14171E 100%)';
const PANEL_BG = 'linear-gradient(180deg,#171A20 0%,#0B0D12 100%)';
...
export default function Admin() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'radial-gradient(120% 100% at 50% 0%, #0A1B4A 0%, #070A14 45%, #08090D 100%)', fontFamily: BODY, color: T.text }}>
      <style>{`
...
function Panel({ adminKey, onLogout }: { adminKey: string; onLogout: () => void }) {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', flexWrap: 'wrap', background: 'radial-gradient(120% 80% at 50% 0%, #0C2358 0%, #0A0D16 55%, #08090D 100%)', backgroundAttachment: 'fixed', fontFamily: BODY }}>
...
const BTN: React.CSSProperties = { background: CHROME_BG, border: 'none', borderRadius: 999, color: '#06121F', fontFamily: HEAD, fontWeight: 700, fontSize: 13.5, letterSpacing: 0.6, padding: '13px 28px', cursor: 'pointer', textTransform: 'uppercase', boxShadow: '0 8px 20px rgba(0,0,0,.3), inset 0 1px 0 rgba(255,255,255,.7)' };
  ...
  return (
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 12, marginBottom: 20 }}>
        {cards.map((c) => (
          <div key={c.label} style={{ background: c.hot ? 'linear-gradient(180deg,#0E1E46 0%,#0B0D12 100%)' : PANEL_BG, border: `1px solid ${c.hot ? '#1E3A8A' : T.line}`, borderRadius: 16, padding: '18px 16px 15px', boxShadow: '0 10px 30px rgba(0,0,0,.4)' }}>
            <p style={{ fontFamily: HEAD, fontWeight: 700, fontSize: 10, letterSpacing: 2.4, textTransform: 'uppercase', color: T.dim, margin: '0 0 8px' }}>{c.label}</p>
            <p style={{ fontFamily: DISPLAY, fontSize: 30, lineHeight: 1.05, color: c.hot ? T.ice : T.white, margin: 0 }}>{c.value}</p>
            {c.meaning && <p style={{ fontFamily: BODY, fontSize: 12.5, color: T.dim, margin: '5px 0 0' }}>{c.meaning}</p>}
          </div>
        ))}
      </div>

      {conv != null && (
        <div style={{ background: 'rgba(17,64,240,0.10)', border: `1px solid rgba(143,211,255,0.28)`, borderRadius: 12, padding: '12px 16px', marginBottom: 24, maxWidth: 520, fontSize: 13.5, color: T.text }}>
